...
use crate::qag_vec8_integration_result::ResultVec8;
use crate::qag_vec8_integrator_result::QagVec8IntegratorResult;
...
impl QagVec8Simd {
    pub fn qintegrate(&self, f : &FnVec8, a : f64, b : f64, epsabs : f64, epsrel : f64)
                      -> QagVec8IntegratorResult {

        if epsabs <= 0.0 && epsrel < 0.5e-28_f64.max(50.0 * EPMACH) {
            return QagVec8IntegratorResult::new_error(ResultState::Invalid)
        }
        ...
        let mut abserr = f64x8::splat(0.0);
        let mut defabs = f64x8::splat(0.0);
        let mut resabs = f64x8::splat(0.0);
        let mut result_list = vec![];

        //let qk21 = Qk21Vec4Simd {};
        let qk61 = Qk61Vec8Simd {};
        ...
        result_list.push(ResultVec8::new(a,b,result,abserr));
        ...
        for k in 0..len{
            ...
            if result_list[0].error[k] <= 50.0 * EPMACH * defabs[k] && result_list[0].error[k] > errbnd[k] {
                return QagVec8IntegratorResult::new_error(ResultState::BadTolerance)
            }
        }
        ...
        if condition1(&result_list[0].error,&errbnd,&resabs) {
            ...
            return QagVec8IntegratorResult::new(result_list[0].result.clone(), result_list[0].error.clone(),
                                                neval, result_list, last)
        }

        if self.limit == 1 {
            return QagVec8IntegratorResult::new_error(ResultState::MaxIteration)
        }
        ...
        let mut area1 = f64x8::splat(0.0);
        let mut error1 = f64x8::splat(0.0);
        let mut area2 = f64x8::splat(0.0);
        let mut error2 = f64x8::splat(0.0);
        let mut defab1 = f64x8::splat(0.0);
        let mut defab2 = f64x8::splat(0.0);

        let mut area12 = f64x8::splat(0.0);
        let mut erro12 = f64x8::splat(0.0);

        let epsabs_simd = f64x8::splat(epsabs);
        let epsrel_simd = f64x8::splat(epsabs);


        while last <= self.limit {

            let last_simd = f64x8::splat(last as f64);
                ...
                if result_list[k].error > epsabs_simd.simd_max(epsrel_simd * area.abs()) / last_simd{
                    ...
                    if errsum > errbnd {
                        ...
                        if a1.abs().max(b2.abs()) <= (1.0 + 100.0 * EPMACH) * (a2.abs() + 1000.0 * UFLOW) {
                            return QagVec8IntegratorResult::new_error(ResultState::BadFunction)
                        }
                    ...
                    result_list[k] = ResultVec8::new(a1, b1, area1, error1);
                    result_list.push(ResultVec8::new(a2, b2, area2, error2));
                }
                if ! (errsum > errbnd) {
                    break
                }
        ...
        QagVec8IntegratorResult::new(result, errsum, neval, result_list, last)
    }
}
...
pub fn condition1(a : &Simd<f64,8>, b : &Simd<f64,8>, c : &Simd<f64,8>) -> bool{
    for k in 0..a.lanes(){
...
pub fn condition2(a : &Simd<f64,8>, b : &Simd<f64,8>, c : f64, d : f64, e : f64) -> bool{
    for k in 0..a.lanes(){
...
mod tests {
    ...
    use crate::qage_8vec_simd::QagVec8Simd;
    ...
    fn test(){
        ...
        let qag_vec3 = QagVec8Simd{key,limit};
        ...
        let fun2 = FnVec8{components : [Box::new(f1),Box::new(f2),Box::new(f3),Box::new(f4),
            Box::new(f1),Box::new(f2),Box::new(f3),Box::new(f4)]};
