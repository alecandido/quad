...
impl Qag_1dvec_parall {
    pub fn qintegrate(&self, fun : &FnVecPa , a : f64, b : f64, epsabs : f64, epsrel : f64)
        ...
        while last <= self.limit {
            ...
            rayon::scope(|s| {
                for component in result_list.clone() {
                    ...
                    if comp.error > epsabs.max(epsrel * area_temp.abs()) / last as f64 {
                        ...
                        s.spawn(move |_| {
                            println!("{:?}",current_thread_index());
                            let mut new_interval = new_interval.lock().unwrap();
                            *new_interval += 1;
                            drop(new_interval);
                            let f = func.components[0].clone();
                            ...
                            *errbnd = epsabs.max(epsrel * area.abs());
                            ...
                            if defab1 == error1 || defab2 == error2 {} else {
                                ...
                                if (component.result - area12).abs() <= 0.00001 * area12.abs() && erro12 >= 0.99 * component.error {
                                    *(iroff1.lock().unwrap()) += 1;
                                    drop(iroff1);
                                    iroff1_temp += 1;

                                }
                                if last > 10 && erro12 > component.error {
                                    *(iroff2.lock().unwrap()) += 1;
                                    drop(iroff2);
                                    iroff2_temp += 1;
                                }
                            }
                            ...
                            if errsum_temp > errbnd_temp {
                                ...
                                if iroff1_temp >= 6 || iroff2_temp >= 20 {
