...
#[derive(Clone)]
pub struct Qag_1dvec_parall_2thread {
    pub key : i32,
    pub limit : usize,
}
...
impl Qag_1dvec_parall_2thread {
    pub fn qintegrate(&self, fun : &FnPa , a : f64, b : f64, epsabs : f64, epsrel : f64)
        ...
        while last <= self.limit {
            ...
            let (res1,res2) = result_list.as_mut_slice().split_at_mut(len/2);

            let ((mut a,area1,error1),(mut b, area2, error2)) = join(|| {
            ...
            last += a.len() + b.len();
            result_list.append(&mut a);
            result_list.append(&mut b);
