...
for a in var:
    f = lambda x : (math.cos(x),math.sin(x),math.cos(x),math.sin(x),math.cos(x),math.sin(x),math.cos(x),math.sin(x))
    epsabs = 1.0e-3
    epsrel = 0.0
    a = 0.0
    b = 10000.0
    ...
    start = time.time()
    res = quad.qag_array(f,a,b,epsabs,epsrel,6,10000,8)
    end = time.time()
    t1 += end - start

    start = time.time()
    res = quad.qag_vec(f,a,b,epsabs,epsrel,6,10000)
    end = time.time()
    t2 += end - start
