...
pub const WGK1: Simd<f64,16> = Simd::from_array([0.001389013698677007624551591226760,
...
pub const WGK2: Simd<f64,16> = Simd::from_array([0.003890461127099884051267201844516,
...
pub const WG: Simd<f64,16> = Simd::from_array([0.007968192496166605615465883474674,0.018466468311090959142302131912047,
    ...
    0.102852652893558840341285636705415, 0.000000000000000000000000000000000]);
...
impl Qk for Qk61Simd3 {
    fn integrate(&self, f: &dyn Fn(f64) -> f64, a: f64, b: f64, ) -> (f64, f64, f64, f64) {
        ...
        let resk = ((fv1+fv2) * WGK1).reduce_sum() + ((fv3+fv4) * WGK2).reduce_sum();
        let reskh = resk * 0.5;
        let reskhs = Simd::from_array([reskh;16]);
        let mut reskhs2 = reskhs.clone();
        reskhs2[15] = 0.000000000000000000000000000000000;
        let mut resabs = ((fv1.abs()+fv2.abs()) * WGK1).reduce_sum() +
            ((fv3.abs()+fv4.abs()) * WGK2).reduce_sum();
        let resg = ( (fv3+fv4) * WG).reduce_sum();
        let mut resasc = ( WGK1 * ( ( fv1 - reskhs).abs() + (fv2 - reskhs2).abs())).reduce_sum() +
...
pub fn fvec_simd(f : &dyn Fn(f64)->f64, centr : f64, hlgth : f64)
                 -> (Simd<f64,16>,Simd<f64,16>,Simd<f64,16>,Simd<f64,16>){
    (Simd::from_array([f(centr + hlgth * XGK[0]), f(centr + hlgth * XGK[2]),
        ...
        f(centr + hlgth * XGK[16]), f(centr + hlgth * XGK[18]), f(centr + hlgth * XGK[20]),
        f(centr + hlgth * XGK[22]), f(centr + hlgth * XGK[24]),f(centr + hlgth * XGK[26]),
        f(centr + hlgth * XGK[28]), f(centr + hlgth * XGK[30])]),
     Simd::from_array([f(centr + hlgth * XGK[31]),
         f(centr + hlgth * XGK[33]), f(centr + hlgth * XGK[35]), f(centr + hlgth * XGK[37]),
         f(centr + hlgth * XGK[39]), f(centr + hlgth * XGK[41]), f(centr + hlgth * XGK[43]),
         f(centr + hlgth * XGK[45]), f(centr + hlgth * XGK[47]), f(centr + hlgth * XGK[49]),
         f(centr + hlgth * XGK[51]), f(centr + hlgth * XGK[53]),f(centr + hlgth * XGK[55]),
         f(centr + hlgth * XGK[57]), f(centr + hlgth * XGK[59]), 0.0 ]),
     Simd::from_array([f(centr + hlgth * XGK[1]), f(centr + hlgth * XGK[3]),
         f(centr + hlgth * XGK[5]), f(centr + hlgth * XGK[7]), f(centr + hlgth * XGK[9]),
         f(centr + hlgth * XGK[11]), f(centr + hlgth * XGK[13]), f(centr + hlgth * XGK[15]),
         f(centr + hlgth * XGK[17]), f(centr + hlgth * XGK[19]), f(centr + hlgth * XGK[21]),
         f(centr + hlgth * XGK[23]), f(centr + hlgth * XGK[25]), f(centr + hlgth * XGK[27]),
         f(centr + hlgth * XGK[29]), 0.0]),
     Simd::from_array([f(centr + hlgth * XGK[32]),f(centr + hlgth * XGK[34]),
         f(centr + hlgth * XGK[36]), f(centr + hlgth * XGK[38]), f(centr + hlgth * XGK[40]),
         f(centr + hlgth * XGK[42]), f(centr + hlgth * XGK[44]), f(centr + hlgth * XGK[46]),
         f(centr + hlgth * XGK[48]), f(centr + hlgth * XGK[50]), f(centr + hlgth * XGK[52]),
         f(centr + hlgth * XGK[54]), f(centr + hlgth * XGK[56]), f(centr + hlgth * XGK[58]),
         f(centr + hlgth * XGK[60]), 0.0]))
