...
impl Qag_1dvec_parall_8thread {
    pub fn qintegrate(&self, fun : &FnPa , a : f64, b : f64, epsabs : f64, epsrel : f64)
        ...
        while last <= self.limit {
            ...
            let (res1,res5) = result_list.as_mut_slice().split_at_mut(len/2);
            let (res1, res3) = res1.split_at_mut(len/4);
            let (res5, res7) = res5.split_at_mut(len/4);
            let (res1, res2) = res1.split_at_mut(len/8);
            let (res3, res4) = res3.split_at_mut(len/8);
            let (res5, res6) = res5.split_at_mut(len/8);
            let (res7, res8) = res7.split_at_mut(len/8);

            let ((((mut vec1,area1,error1),(mut vec2, area2, error2)),
                ((mut vec3,area3,error3),(mut vec4, area4, error4))),
                (((mut vec5,area5,error5),(mut vec6, area6, error6)),
                    ((mut vec7,area7,error7),(mut vec8, area8, error8)))) =
                join(|| {join(||
                                  {join( ||{
                                  ...
                                  {join( ||{
                                      ...
                                      for component in res5 {
                                          ...
                                          if component.error > epsabs.max(epsrel * area.abs()) / last as f64 {
                                              ...
                                              areat5 += area1 + area2;
                                              errort5 += error1 + error2;

                                              *component = Result::new(a1, b1, area1, error1);
                                              new_contributes.push(ArcResult::new(a2, b2, area2, error2));
                                          }
                                          else{
                                              areat5 += component.result;
                                              errort5 += component.error;
                                          }
                                      }
                                      (new_contributes, areat5, errort5)
                                  }
                                         ,||{
                                          let mut new_contributes = vec![];
                                          let mut areat6 = 0.0;
                                          let mut errort6 = 0.0;
                                          for component in res6 {
                                              let mut component = component.inner.lock().unwrap();
                                              ...
                                              if component.error > epsabs.max(epsrel * area.abs()) / last as f64 {
                                                  ...
                                                  areat6 += area1 + area2;
                                                  errort6 += error1 + error2;

                                                  *component = Result::new(a1, b1, area1, error1);
                                                  new_contributes.push(ArcResult::new(a2, b2, area2, error2));
                                              }
                                              else{
                                                  areat6 += component.result;
                                                  errort6 += component.error;
                                              }
                                          }
                                          (new_contributes, areat6, errort6)
                                  ...
                                  {join( ||{
                                      ...
                                      for component in res7 {
                                          ...
                                          if component.error > epsabs.max(epsrel * area.abs()) / last as f64 {
                                              ...
                                              areat7 += area1 + area2;
                                              errort7 += error1 + error2;

                                              *component = Result::new(a1, b1, area1, error1);
                                              new_contributes.push(ArcResult::new(a2, b2, area2, error2));
                                          }
                                          else{
                                              areat7 += component.result;
                                              errort7 += component.error;
                                          }
                                      }
                                      (new_contributes, areat7, errort7)
                                  }
                                         ,||{
                                          let mut new_contributes = vec![];
                                          let mut areat8 = 0.0;
                                          let mut errort8 = 0.0;
                                          for component in res8 {
                                              let mut component = component.inner.lock().unwrap();
                                              ...
                                              if component.error > epsabs.max(epsrel * area.abs()) / last as f64 {
                                                  ...
                                                  areat8 += area1 + area2;
                                                  errort8 += error1 + error2;

                                                  *component = Result::new(a1, b1, area1, error1);
                                                  new_contributes.push(ArcResult::new(a2, b2, area2, error2));
                                              }
                                              else{
                                                  areat8 += component.result;
                                                  errort8 += component.error;
                                              }
                                          }
                                          (new_contributes, areat8, errort8)
            ...
            last += vec1.len() + vec2.len() + vec3.len() + vec4.len() +
                vec5.len() + vec6.len() + vec7.len() + vec8.len();
            result_list.append(&mut vec1);
            result_list.append(&mut vec2);
            result_list.append(&mut vec3);
            result_list.append(&mut vec4);
            result_list.append(&mut vec5);
            result_list.append(&mut vec6);
            result_list.append(&mut vec7);
            result_list.append(&mut vec8);
            ...
            errsum = error1 + error2 + error3 + error4 + error5 + error6 + error7 + error8;
