...
mod tests {
    ...
    fn test(){
        ...
        let qk = Qk61 {};
        let qk_vec = Qk614Vec {};
        let qk_vec_gen = Qk61VecNorm {};
        let qk_vec_gen2 = Qk61VecNorm2{};
        let f = |x:f64| [f1(x),f2(x),f3(x),f4(x)];
        let fvec  = |x:f64| vec![f1(x),f2(x),f3(x),f4(x)];
        let fvec2 = |x:f64| [f1(x),f2(x),f3(x),f4(x)];
        ...
        let mut res_vec_gen = (vec![],0.0,0.0);
        let mut res_vec_gen2 = ([0.0;4],0.0,0.0);
        ...
        for k in 0..max {
            ...
            let start = Instant::now();
            res_vec_gen2 = qk_vec_gen2.integrate(&fvec2, a, b);
            println!("vec gen2 {:?}", start.elapsed());

            if k == max-1 {
                println!("normal {:?},{:?},{:?},{:?}",res1,res2,res3,res4);
                println!("vec {:?}",res_vec);
                println!("vec gen {:?}", res_vec_gen);
                println!("vec gen {:?}", res_vec_gen2);
            }
        }
