...
impl Qk61VecScipyNd {
    pub fn integrate<F>(&self, f: F, a: f64, b: f64, n : usize )-> (Array1<f64>, f64, f64)
    where F : Fn(f64)->Array1<f64>{
        ...
        let fc  = f(centr);
        let dim = fc.len();
        let mut resg : Array1<f64> = Array1::zeros(dim);
        let mut resk : Array1<f64> = fc.clone() * WGK[30];
        let mut resabs : Array1<f64> = ndabs(&resk);
        ...
        for j in 1..16 {
            ...
            resabs = resabs + WGK[jtw - 1] * (ndabs(&fval1) + ndabs(&fval2));
            fv1.push(fval1.clone());
            fv2.push(fval2.clone());
            let fsum  = fval1 + fval2;
            resg =  resg + WG[j - 1] * fsum.clone();
        ...
        for j in 1..16 {
            ...
            let fval1  = f(centr - absc);
            let fval2  = f(centr + absc);
            resabs = resabs + WGK[jtwm1 - 1] * (ndabs(&fval1) + ndabs(&fval2));
            fv1.push(fval1.clone());
            fv2.push(fval2.clone());
            let fsum  = fval1 + fval2;
        ...
        let reskh = resk * 0.5;
        let mut resasc = WGK[30] * ndabs(&(fc - reskh.clone()));

        for k  in 0..fv1.len(){
            fv1[k] = fv1[k] - reskh.clone();
            fv2[k] = fv2[k] - reskh.clone();
        }

        for j in 1..16 {
            resasc = resasc + WGK[2*j - 1] * (ndabs(&(fv1[j - 1])) + ndabs(&(fv2[j - 1]))) +
                WGK[2*(j - 1)] * (ndabs(&(fv1[j + 14])) +
                ndabs(&(fv2[j + 14])))  ;
        }
        ...
        let resasc = (resasc * resasc).sum().sqrt();
        let resabs = (resabs * resabs).sum().sqrt();

        let abserr = ndabs(&((resk - resg) * hlgth));
        let mut abserr = (abserr * abserr).sum().sqrt();
...
pub fn ndabs(a : &Array1<f64>) -> Array1<f64>{
    let mut b = a.clone();
    let dim = b.len();
    for k in 0..dim{
        b[k] = b[k].abs();
...
mod tests {
    ...
    use crate::qk61_vec_scipy_nd::ndabs;
    use crate::qk::Qk;

    #[test]
    fn test(){
        let arr1 = array![-1.0,-2.0,-3.0];
        let arrabs = ndabs(&arr1);
        println!("{:?}",arrabs);
    }
}
