...
#[derive(Clone)]
pub struct QagVecNosortFindmax {
    pub key : i32,
    pub limit : usize,
}
...
impl QagVecNosortFindmax {
    pub fn qintegrate(&self, f : &dyn Fn(f64)->[f64;4], a : f64, b : f64, epsabs : f64, epsrel : f64)
        ...
        for last  in 2..self.limit + 1 {
            ...
            for k in 0..4{
                if defab1[k] == error1[k] || defab2[k] == error2[k] {} else {
                    if (rlist[maxerr][k] - area12[k]).abs() <= 0.00001 * area12[k].abs() && erro12[k] >= 0.99 * errmax[k] {
                        iroff1[k] += 1;
                    }
                    if last > 10 && erro12[k] > errmax[k] { iroff2[k] += 1; }
                }
    ...
    fn test(){
        ...
        for k in 0..1 {
            ...
            if k == 0{
                ...
                println!("vec {:?}",res_vec.result_state);
            }
        }
