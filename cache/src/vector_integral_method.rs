...
pub trait VecIntegralMethod {
    fn integrate_non_uniform(&self, funct : & FunctVector, number_of_points : Vec<i32>) -> Vec<f64>;
    fn integrate_non_uniform_parallel(&self, funct: & FunVector , number_of_points_vec: Vec<i32>) -> Vec<f64>;
    fn errors_non_uniform(&self, funct: & FunctVector, number_of_points : Vec<i32>) -> Vec<f64> ;
    fn errors_non_uniform_parallel(&self, funct: & FunVector, number_of_points : Vec<i32>) -> Vec<f64> ;
    fn integrate_uniform(&self, funct : & FunctVector, number_of_points : i32) -> Vec<f64> {
        let number_of_points_vec = vec![number_of_points;funct.components.len()];
        self.integrate_non_uniform(funct,number_of_points_vec)
    }
    fn integrate_uniform_parallel(&self, funct: & FunVector , number_of_points: i32) -> Vec<f64>{
        let number_of_points_vec = vec![number_of_points;funct.components.len()];
        self.integrate_non_uniform_parallel(funct,number_of_points_vec)
    }
    fn errors_uniform(&self, funct: & FunctVector, number_of_points : i32) -> Vec<f64> {
        let number_of_points_vec = vec![number_of_points;funct.components.len()];
        self.errors_non_uniform(funct,number_of_points_vec)
    }
    fn errors_uniform_parallel(&self, funct: & FunVector, number_of_points : i32) -> Vec<f64> {
        let number_of_points_vec = vec![number_of_points;funct.components.len()];
        self.errors_non_uniform_parallel(funct,number_of_points_vec)
    }
    fn relative_error_uniform(&self, funct : & FunctVector, number_of_points : i32) -> Vec<f64> {
        let dimension = funct.components.len();
        let mut relative_errors :Vec<f64> = vec![0.0;dimension];
        for k in 0..dimension {
            relative_errors[k] = (self.errors_uniform(funct, number_of_points)[k] / self.integrate_uniform(funct, number_of_points)[k]).abs();
        }
        relative_errors
    }
    fn relative_error_non_uniform(&self, funct : & FunctVector, number_of_points : Vec<i32>) -> Vec<f64> {
        let dimension = funct.components.len();
        let mut relative_errors :Vec<f64> = vec![0.0;dimension];
        for k in 0..dimension {
    ...
    fn relative_error_uniform_parallel(&self, funct : & FunVector, number_of_points : i32) -> Vec<f64> {
        let dimension = funct.components.len();
        let mut relative_errors: Vec<f64> = vec![0.0; dimension];
        for k in 0..dimension {
    ...
    fn relative_error_non_uniform_parallel(&self, funct : & FunVector, number_of_points : Vec<i32>) -> Vec<f64> {
        let dimension = funct.components.len();
        let mut relative_errors :Vec<f64> = vec![0.0;dimension];
        for k in 0..dimension {
...
impl VecIntegralMethod for VecRectangular {
    fn integrate_non_uniform(&self, funct: & FunctVector, number_of_points: Vec<i32>) -> Vec<f64> {
        let dimension = funct.components.len();
        let mut results :Vec<f64> = vec![0.0;dimension];
        for k in 0..dimension {
            let mut tot : f64 = funct.components[k].evaluate(&0.0);
            let m = number_of_points[k] as f64 ;
    ...
    fn integrate_non_uniform_parallel(&self, funct: & FunVector , number_of_points_vec: Vec<i32>) -> Vec<f64> {
        ...
        for k in 0..dimension {
            ...
            let handle = thread::spawn( move || {
                ...
                let mut tot: f64 = function.evaluate(&0.0);
    ...
    fn errors_non_uniform(&self, funct: & FunctVector, number_of_points : Vec<i32>) -> Vec<f64>{
        let dimension = funct.components.len();
        let mut errors :Vec<f64> = vec![0.0;dimension];
        for k in 0..dimension {
            let number_of_points_float = number_of_points[k] as f64;
            let max: f64 = funct.components[k].first_derivative_abs_max();
    ...
    fn errors_non_uniform_parallel(&self, funct: & FunVector, number_of_points : Vec<i32>) -> Vec<f64>{
        ...
        for k in 0..dimension {
            ...
            let handle = thread::spawn( move || {
                let function = pointer.lock().unwrap();
                let max: f64 = function.first_derivative_abs_max();
...
impl VecIntegralMethod for VecTrapezoiadal {
    fn integrate_non_uniform(&self, funct: & FunctVector, number_of_points: Vec<i32>) -> Vec<f64> {
        ...
        let mut results :Vec<f64> = vec![0.0;dimension];
        for k in 0..dimension {
            let mut tot: f64 = 0.5 * (funct.components[k].evaluate(&0.0) + funct.components[k].evaluate(&1.0));
            let m = number_of_points[k] as f64;
            for i in 1..number_of_points[k] {
                let j = i as f64;
                tot += funct.components[k].evaluate(&(j / m));
    ...
    fn integrate_non_uniform_parallel(&self, funct: & FunVector , number_of_points_vec: Vec<i32>) -> Vec<f64> {
        ...
        for k in 0..dimension {
            ...
            let handle = thread::spawn( move || {
                let function = pointer.lock().unwrap();
                let mut tot: f64 = 0.5 * ( function.evaluate(&0.0) + function.evaluate(&1.0) );
                let m = number_of_points as f64;
                for i in 1..number_of_points {
                    let j = i as f64;
                    tot += function.evaluate(&(j / m));
    ...
    fn errors_non_uniform(&self, funct: & FunctVector, number_of_points : Vec<i32>) -> Vec<f64>{
        ...
        for k in 0..dimension {
            let number_of_points_float = number_of_points[k] as f64;
            let max: f64 = funct.components[k].second_derivative_abs_max();
    ...
    fn errors_non_uniform_parallel(&self, funct: & FunVector, number_of_points : Vec<i32>) -> Vec<f64>{
        ...
        for k in 0..dimension {
            ...
            let handle = thread::spawn( move || {
                let function = pointer.lock().unwrap();
                let max: f64 = function.second_derivative_abs_max();
...
impl VecIntegralMethod for VecSimpson1 {
    fn integrate_non_uniform(&self, funct: & FunctVector, number_of_points: Vec<i32>) -> Vec<f64> {
        ...
        let mut results :Vec<f64> = vec![0.0;dimension];
        for k in 0..dimension {
            let mut tot: f64 = funct.components[k].evaluate(&0.0) - funct.components[k].evaluate(&1.0);
            let m = number_of_points[k] as f64;
    ...
    fn integrate_non_uniform_parallel(&self, funct: & FunVector , number_of_points_vec: Vec<i32>) -> Vec<f64> {
        ...
        for k in 0..dimension {
            ...
            let handle = thread::spawn( move || {
                let function = pointer.lock().unwrap();
                let mut tot: f64 = function.evaluate(&0.0) - function.evaluate(&1.0);
                let m = number_of_points as f64;
    ...
    fn errors_non_uniform(&self, funct: & FunctVector, number_of_points : Vec<i32>) -> Vec<f64> {
        ...
        for k in 0..dimension {
            let number_of_points_float = number_of_points[k] as f64;
            let max: f64 = funct.components[k].fourth_derivative_abs_max();
    ...
    fn errors_non_uniform_parallel(&self, funct: & FunVector, number_of_points : Vec<i32>) -> Vec<f64>{
        ...
        for k in 0..dimension {
            ...
            let handle = thread::spawn( move || {
                let function = pointer.lock().unwrap();
                let max: f64 = function.fourth_derivative_abs_max();
