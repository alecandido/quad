...
mod tests {
    ...
    fn test(){
        ...
        let fv_par = FnVecGen{
            components : Arc::new(|x:f64| [x.cos(),x.sin(),x.cos()+x.sin(),-2.0 * (x.cos()+x.sin())]),
        };
        ...
        let qag_vec_norm3_par = QagVecNormPar{key,limit};
        ...
        for k in 0..max {
            ...
            let start = Instant::now();
            res_par= qag_vec_norm3_par.qintegrate(fv_par.clone(),a,b,epsabs,epsrel);
            println!("scipy par time : {:?}",start.elapsed());

            if k == max-1{
                println!("{:?}",res);
                println!("{:?}",res2.unwrap());
                println!("{:?}",res3.unwrap());
                println!("{:?}",res_par.unwrap());
