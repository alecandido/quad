...
impl Qk61VecNorm {
    pub(crate) fn integrate(&self, f: &dyn Fn(f64) -> Vec<f64>, a: f64, b: f64, ) -> (Vec<f64>, f64, f64) {
        ...
        let resabs = norm_s(&resabs, dhlgth);
        let resasc = norm_s(&resasc, dhlgth);
...
pub fn err(v : &[f64], w : &[f64] , s : f64) -> f64{
    let mut err = 0.0;
    for (i,j) in zip(v,w){
        err += ((i - j).abs()* s).powi(2) ;
    }
    err = err.sqrt();
    err
}
